...
import pygame
from Tools import Background, Mouse, Sprite_Mouse_Location, load_image, Particle, Particle_2, Atom
import Map as Map
...
def three1():
    ...
    a = ['Скриплеры - волшебные существа',
         'Они издавна помогают всем людям',
         'Но на их дом напал злобный пылевой монстр',
         'Помоги им и защити их дом!',
         'Чтобы атаковать, нажми кнопку АТАКОВАТЬ',
         'И накопи энергию волшебных вещей']
    w00, h00 = 460, 430
    ...
    def create_particles(position, list_pictures_0, k=False):
        ...
        for _ in range(particle_count):
            if k:
                Particle((position[0] - 10, position[1]), choice(numbers), choice(numbers), mini_game_1, 0.5,
                         screen_rect,
                         list_pictures_0)
            else:
                Particle((position[0] - 10, position[1]), choice(numbers), choice(numbers), all_sprites, 0.5,
                         screen_rect,
                         list_pictures_0)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if a:
                    ...
                    if level == 10001:
                        ...
                        Map.main()
                        sys.exit()
                if event.button == 1 and pygame.sprite.collide_rect(sprite5, mouse_sprite) \
                        and button != 3 and not a:
                    ...
                    pygame.mixer.music.load("sounds/boy.mp3")
                    pygame.mixer.music.play(-1)
                elif event.button == 1 and pygame.sprite.collide_rect(sprite5, mouse_sprite) and not a:
                    ca = True
                    button = 0
